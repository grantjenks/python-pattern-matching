...
pattern matching. I know you offer me `if`/`elif`/`else` statements but I need
...
operators and changing codecs. This module started as a codec hack but those
are hard because they need an ecosystem of emacs-modes, vim-modes and the like
to really be convenient.
...
It might have read better if I'd used an `if` statement rather than `with`. In
that case you would read:
...
this to save keystrokes so that works against that goal.

Now, regarding the `quote` function call that is not really a function call at
all. You can only put the name of a variable within that call and if you do so
it won't be evaluated at runtime. Instead, it will be bound to its matching
...
which might look like: `q"blah"`. It's rare that I see a
[C++ feature](http://en.wikipedia.org/wiki/C%2B%2B11#User-defined_literals "User-defined Literals")
and look on with envy. This topic more commonly arises when people want a
syntax for stating an `OrderedDict` in an expression. Maybe
...
I'm not convinced this is in fact better. But it's interesting. I think you
could do nearly this in `macropy` if you were willing to organize your code
for the import hook to work.
